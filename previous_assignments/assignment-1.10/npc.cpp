...
void gen_monsters(dungeon_t *d, uint32_t nummon, uint32_t game_turn)
{
  ...
  for (i = 0; i < nummon; i++) {
	...
	// generate a monster from a random description
	m = d->monster_descriptions[rand_range(0, d->monster_descriptions.size() - 1)]
								  .generate_instance(d);
...
void gen_rand_monsters(dungeon_t *d, uint32_t nummon, uint32_t game_turn)
{
  uint32_t i;
  npc *m;
  uint32_t room;
