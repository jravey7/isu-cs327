...
char get_object_symbol(object_type_t type)
{
...
object * object_description::generate_instance(){
	char symbol = get_object_symbol(this->type);

	object * ret = new object(this->name, this->description, this->type, this->color, this->hit.roll(),
		this->dodge.roll(), this->defence.roll(), this->weight.roll(), this->speed.roll(),
		this->attribute.roll(), this->value.roll(), this->damage, symbol);

	return ret;
}

npc * monster_description::generate_instance(dungeon_t * d) {
	npc * ret = new npc();

	ret->position[dim_y] = 0;
	ret->position[dim_x] = 0;
	ret->symbol = this->symbol;
	ret->speed = this->speed.roll();
	ret->next_turn = 0;
	ret->alive = 1;
	ret->hitpoints = this->hitpoints.roll();
	ret->damage = this->damage;
	ret->sequence_number = ++d->character_sequence_number;
	ret->characteristics = this->abilities;
	ret->have_seen_pc = 0;
	ret->pc_last_known_position[dim_y] = 0;
	ret->pc_last_known_position[dim_x] = 0;
